...
AOS.init();// ========================
// dark and light mode===============================================
let a=document.querySelector(".top-nav__switch-input"),n=document.querySelector(".top-nav__theme-text"),s=document.querySelector(".top-nav__theme-icon");a.addEventListener("change",t=>{let e=t.target.checked;l(e?"dark":"light")});const i=window.document.documentElement,c=window.localStorage.getItem("theme");if(c)l(c);else{let t=function(){let t=window.matchMedia("(prefers-color-scheme: dark)").matches;return t?"dark":"light"}();l(t)}function l(t){window.localStorage.setItem("theme",t),"dark"===t?(a.checked=!0,n.textContent="Dark Mode",s.classList.replace("fa-sun","fa-moon"),i.dataset.theme=t):(n.textContent="Light Mode",s.classList.replace("fa-moon","fa-sun"),i.setAttribute("data-theme",t))}// ========================
...
// ========================
let j=S(0,40),V=0;$("#cart")&&j.then(t=>{let e=g("cart"),o=[];for(let a=0;a<e.length;a++){let n=t.items.filter((t,o)=>t.id==e[a].id);n&&(n[0].count=e[a].count||1),o.push(n[0]);// console.log(cartArray);
...
j.then(t=>{let e=g("wish"),o=[];for(let a=0;a<e.length;a++){let n=t.items.filter((t,o)=>t.id==e[a].id);o.push(n[0])}console.log(o);let a=[...new Set(o)];$(a).each((t,e)=>{price="NOT_FOR_SALE"==e.saleInfo.saleability?"Not Available":`EGP${e.saleInfo.listPrice?.amount.toFixed(2)}`,// =========================================================
// let singlePrice = book[0].id;
...
// getPromiseData(0, 20);
function(t){e(`https://www.googleapis.com/books/v1/volumes/${t}`).then(t=>{let e=`EGP${t?.saleInfo?.listPrice?.amount.toFixed(2)}`;"NOT_FOR_SALE"==t.saleInfo.saleability&&(e="NOT AVAILABLE"),(Z||Q||tt||X||te||to||ta||tn)&&(Z.innerText=t.volumeInfo.title,// ===================
Q.src=t.volumeInfo.imageLinks.small,t.volumeInfo.industryIdentifiers.forEach(t=>{// ===================
tt.innerHTML+=`<p>${t.type.toUpperCase()}:<span class="book__isbn ms-2 fw-bold">${t.identifier}</span></p>`}),// ===================
ta.innerText=`${e}`,// ===================
X.innerText=t.volumeInfo.title,// ===================
to.innerText=t.volumeInfo.authors,"NOT_FOR_SALE"!==t.saleInfo.saleability?(tn.innerHTML='<i class="fa-solid fa-circle-check text-success"></i> Available',tn.classList.add("text-success")):(tn.innerHTML='<i class="fa-solid fa-circle-xmark text-danger "></i> Not Available',tn.classList.add("text-danger"))),tc&&(tc.innerHTML=`<button
        ...
        >
        <i class="fa-solid fa-heart"></i> Add to wish
        </button>`),(ts||ti)&&(ts.innerHTML=t.volumeInfo.description),// ===================
ti.innerHTML=`
...
// console.log(item.id);
d(t),m(t)});// ddd();
}(null==(t=RegExp("[?&]productId=([^&#]*)").exec(window.location.href))?null:t[1]||0);const Z=document.getElementById("book-name"),Q=document.querySelector(".book__img"),X=document.querySelector(".book__name"),tt=document.querySelector(".book__isbn"),te=document.querySelector(".book__isbn13"),to=document.querySelector(".book__author"),ta=document.querySelector(".book__price"),tn=document.querySelector(".book__availability"),ts=document.querySelector(".book__description-text"),ti=document.querySelector(".information__details"),tc=document.querySelector(".book__buttons");// =========================
// ADDING BESTSELLERS CARDS=====================================================
let tl=Math.floor(34*Math.random());const tr=document.getElementById("related-cards");o(S(tl,tl+5),tr);// ========================================
//# sourceMappingURL=index.28f67da3.js.map
